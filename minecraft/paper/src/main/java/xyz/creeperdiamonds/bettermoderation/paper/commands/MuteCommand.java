...
public class MuteCommand implements CommandExecutor {
    ...
    public MuteCommand(BetterModerationPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("bettermoderation.mute")) {
            sender.sendMessage("§cYou do not have permission to mute players.");
            return true;
        }
        ...
        sender.sendMessage("§aMuting §f" + targetName + "§a...");

        plugin.getBackendClient()
                .issuePunishment(uuid, PunishmentType.MUTE, reason, issuedBy, finalExpiresAt)
                .thenAccept(punishment -> {
                    if (punishment == null) {
                        sender.sendMessage("§cFailed to mute §f" + targetName + "§c — backend error.");
                        return;
                    }
                    String expStr = finalExpiresAt == null ? "permanent" : "expires at " + finalExpiresAt;
                    sender.sendMessage("§aMuted §f" + targetName + " §afor: §f" + reason + " §7(" + expStr + ")");
